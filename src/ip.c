...
static int hostport(char *str, addr **sa)
{
      ...
      else
      {
            ...
            if (getaddrinfo(host, port, &hint, sa))
                  return -3;
            return host ? 0 : 1;
      }
}
...
static int open_tcp(addr *sa)
{
      ...
      if (setnonblock(sock) < 0)
            fatal("Cannot make socket non-blocking\n");

      //  Bind to port
      if (bind(sock, sa->ai_addr, sizeof(struct sockaddr)) < 0)
            fatal("Cannot bind socket to TCP port\n");
      if (verbose > 0)
            info("TCP socket %d bound to port\n", sock);
...
static int connect_tcp(addr *sa)
{
      ...
      sock = socket(sa->ai_family, sa->ai_socktype, sa->ai_protocol);
      if (sock < 0)
            fatal("Cannot open socket for connect\n");
      if (verbose > 1)
            info("TCP connect socket %d\n", sock);

      //  Set TCP keepalive
      if (setkeepalive(sock) < 0)
            fatal("Cannot make socket TCP keepalive\n");
...
void TCPopen(unsigned int *n, char *name, unsigned int *nn)
{
      ...
      if (WSAStartupError)
            fatal("Cannot initialize WinSock %d\n", WSAStartupError);
#endif
      ...
      else
      {
            ...
            if (recv_tcp(sock, &mi, 2))
                  fatal("TCPRECV error MI\n");
            rev = (mi != 1);
            ...
            if (send_tcp(sock, &runtime, 4))
                  fatal("TCPSEND error\n");
            if (send_tcp(sock, text, LENTEXT))
                  fatal("TCPSEND error\n");
            if (send_tcp(sock, &time_index, 4))
                  fatal("TCPSEND error\n");
      }
}
/*
...
void TCPpost(int *id, int *tag, int *l, int *n)
{
      ...
      else
      {
            ...
            if (N > node[*id].max[*tag])
            {
                  ...
                  if (!node[*id].buf[*tag])
                        fatal("TCPPOST realloc error tag %d size %d\n", *tag, N);
            }
...
void SetDir(const char *dir)
{
      ...
      if (pdir)
      {
            ...
            {
                  fatal("Cannot change directory to %s when it is already there\n", dir);
            }
      }
      else if (ENOENT == errno)
      {
            ...
            if (mkdir(dir, 0777) == 0)
            {
                  ...
                  {
                        fatal("Cannot change directory after creating it to %s\n", dir);
                  }
            }
            else
            {
                  fatal("Cannot create directory to %s\n", dir);
            }
      }
      else
      {
            /* opendir() failed for some other reason. */
            fatal("opendir(%s) failed for some other reason\n", dir);
      }
}
